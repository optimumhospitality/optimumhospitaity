...
export default function TeamDetail({ member }: TeamDetailProps) {
  return (
    <>
      ...
      <section className="bg-primary pt-16 md:pt-18">
        <div className="max-w-[1440px] mx-auto flex flex-col md:flex-row">
          {/* Mobile Back Button - Above image */}
          <Link
            to="/team"
            className="md:hidden flex items-center gap-2 text-white text-md font-light px-4 py-3 mt-4"
          >
          ...
          <div className="w-full max-w-[350px] mx-auto px-4 py-4 md:max-w-none md:mx-0 md:h-full md:w-[350px] lg:w-[520px] md:p-0 flex-shrink-0">
            <img
              src={member.image}
              alt={member.name}
          ...
          <div className="flex-1 text-white flex flex-col justify-center items-center md:items-start text-center md:text-left px-4 sm:px-6 md:px-8 lg:px-16 pt-3 pb-8 md:pb-0 md:py-0">
            {/* Desktop Back Button - Above name, in row */}
            <Link
              to="/team"
              className="hidden md:inline-flex items-center gap-2 text-white font-light text-md mb-4"
            >
              <ChevronLeft className="w-5 h-5" />
              back to team
            </Link>

            <h1 className="mt-0 md:mt-0 text-2xl sm:text-3xl md:text-4xl lg:text-[40px] font-[500] mb-1">
              {member.name}
            </h1>
          ...
          <div className="px-0">
            ...
            <div className="space-y-4 md:space-y-6">
              {member.about.map((paragraph, index) => (
                <p
                  key={index}
                  className="text-[#393E4F] text-sm sm:text-base md:text-lg lg:text-[20px] leading-relaxed font-[300]"
                >
                  {paragraph}
                </p>
              ))}
